...
import { Photo, PhotoReaction, PhotoComment, File as BackendFile } from '../backend';
...
export function useUploadPhoto() {
  ...
  return useMutation({
    mutationFn: async (photoData: {
      ...
      // Create organized folder structure based on club/team selection
      ...
      console.log('Uploading photo to organized folder path:', folderPath);
      ...
      console.log('Photo uploaded and registered:', {
...
export function useUploadFile() {
  ...
  return useMutation({
    mutationFn: async (fileData: {
      ...
      console.log('Uploading file to organized folder path:', folderPath);
...
// Vault Hooks - Enhanced for organized folder access with proper membership checking
export function useGetVaultFolders() {
  const { actor, isFetching: actorFetching } = useActor();
  const { identity } = useInternetIdentity();

  return useQuery<Array<{ id: string; name: string; type: 'club' | 'team'; photoCount: number; fileCount: number }>>({
    queryKey: ['vaultFolders'],
    queryFn: async () => {
      if (!actor || !identity) throw new Error('Actor or identity not available');
      
      const currentUserPrincipal = identity.getPrincipal().toString();
      const folders: Array<{ id: string; name: string; type: 'club' | 'team'; photoCount: number; fileCount: number }> = [];
      
      console.log('Loading vault folders for user:', currentUserPrincipal);
      
      // Get all clubs and teams to check user access
      const [allClubs, allTeams, clubMemberships, teamMemberships] = await Promise.all([
        actor.getAllClubs(),
        actor.getAllTeams(),
      ...
      for (let i = 0; i < allClubs.length; i++) {
        ...
        if (hasAccess) {
          try {
            ...
            console.log(`Club ${club.name} (${folderId}): ${clubPhotos.length} photos, ${clubFiles.length} files`);
      ...
      for (let i = 0; i < allTeams.length; i++) {
        ...
        if (hasAccess) {
          try {
            ...
            console.log(`Team ${team.name} (${folderId}): ${teamPhotos.length} photos, ${teamFiles.length} files`);
      ...
      console.log('Total vault folders loaded:', folders.length);
